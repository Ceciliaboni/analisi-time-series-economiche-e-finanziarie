...
.circle <- function(win = 1.3, main = "")
{
  ...
  #### Initialize plot
  plot(x = c(-win, win), c(-win, win), type = "n", 
    xlab = "", ylab = "", main = main[1])
...
.arma.roots <- function(fit)
{
  ...
  #### Roots separated between short and seasonal
  ar.r <- polyroot(c(1, -ar))
  ma.r <- polyroot(c(1, ma))
  sar.r <- polyroot(c(1, -sar))
  sma.r <- polyroot(c(1, sma))
  #### All Roots mixed form of parameters
  long <- .sarma2larma(ar = ar, ma = ma, sar = sar, sma = sma, s = S)
  ar.l.r <- polyroot(c(1, -long$ar))
  ma.l.r <- polyroot(c(1, long$ma))
  
  #### Answer
  list(coef = list(ar = ar, ma = ma, sar = sar, sma = sma), 
    coef.long = long, 
    root = list(ar = ar.r, ma = ma.r, sar = sar.r, sma = sma.r),
    root.long = list(ar = ar.l.r, ma = ma.l.r))
}
# ------------------------------------------------------------------------------
