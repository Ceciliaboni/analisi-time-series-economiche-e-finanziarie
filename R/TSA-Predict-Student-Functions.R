  ...
  g = c("id", "sqrt", "log", "log10"))
{
  ...
  ##  J:       (numeric[1]) Number of ex-post forecasts to compute. 
  ##  n.ahead: (numeric[1]) Number of steps ahead (horizon).
  ##  g:       (character[1] or numeric[1]) transformation used. 
  ##           If character, "id", "sqrt", "log", "log10" are currently 
  ##           implemented.
  ##           If numeric, lambda parameter of the Box-Cox transformation.
  ## Value
  ...
  if (J < 1)
  {
    ans <- if (naive == 0)
    {
      rep.int(mean(y), n.ahead)
    }
    else if (naive == 1)
    {
      rep.int(y[nobs], n.ahead)
    }
    else
    {
      rep(x = y[(nobs-S+1):nobs], length.out = n.ahead)
    }
  }
  else
  {
    if (naive == 0)
    {
      n1 <- nobs - J - n.ahead
      n2 <- nobs - n.ahead
      num <- sum(y[1:n1]) + cumsum(y[(n1+1):n2])
      den <- (n1+1) : n2
      ans <- num / den
    }
    else
    {
      ...
      ans <- y[(nobs - J - naive + 1) : (nobs - naive) ]
    }
  }
  #### Transform
  ...
  {
    ans <- InvBoxCox(x = ans, lambda = g, biasadj = FALSE, fvar = NULL)
  }
...
{ 
  ...
  c(ME = ME, MAE = MAE, RMSE = RMSE,  
    MPE = MPE, MAPE = MAPE, RMSPE = RMSPE, MLLE = MLLE,  
    ScMAE = ScMAE, ScRMSE = ScRMSE) # , ScLLE = ScLLE)
}
# ------------------------------------------------------------------------------
  ...
  msg = "")
{
  ...
  # r <- c(0, 1)
  # theta <- R %*% coef - r
  # vtheta <- tcrossprod( R %*% vcov, R)
  # ch <- chol(vtheta)
  # stat <- sum( solve(t(ch), theta)^2 )
  # pv <- 1 - pchisq(df = np, q = stat)
  #### Coef
  ...
  test <- linearHypothesis(model = lm1, 
    hypothesis.matrix = c("(Intercept) = 0", "fit = 0"), vcov. = vcov)
  testHC <- linearHypothesis(model = lm1, 
    hypothesis.matrix = c("(Intercept) = 0", "fit = 0"), vcov. = vcovHC)
  testHAC <- linearHypothesis(model = lm1, 
    hypothesis.matrix = c("(Intercept) = 0", "fit = 0"), vcov. = vcovHAC)
  #### Print
  if ( msg != "" )
  {
    cat(msg)
  ...
  #### Answer
  list(x = test, xHC = testHC, xHAC = testHAC, coef = coef) 
}
# ------------------------------------------------------------------------------
...
function(y, f1, f2, h, loss)
{
  ...
  dm.test(e1 = e1, e2 = e2, h = h, power = 1)
}
# ------------------------------------------------------------------------------
  ...
  g = c("id", "sqrt", "log", "log10"))
{
  ...
  {
    mean <- InvBoxCox(x = mean, lambda = g, biasadj = TRUE, fvar = sd^2)
  }
  ...
  g = c("id", "sqrt", "log", "log10") )
{
  ...
  {
    median <- InvBoxCox(x = median, lambda = g, biasadj = FALSE, fvar = NULL)
    lower <- InvBoxCox(x = lower, lambda = g, biasadj = FALSE, fvar = NULL)
    upper <- InvBoxCox(x = upper, lambda = g, biasadj = FALSE, fvar = NULL)
  }
